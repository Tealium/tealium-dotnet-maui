...
namespace Tealium.Platform.iOS
{
	...
	interface TealiumWrapper
	{
		...
		// -(void)gatherTrackData:(void (^ _Nonnull)(NSDictionary<NSString *,id> * _Nonnull))completion;
		[Export ("gatherTrackData:")]
		void GatherTrackData (Action<NSDictionary<NSString, NSObject>> completion);

		// -(void)getWebView:(void (^ _Nonnull)(WKWebView * _Nonnull))completion;
		[Export ("getWebView:")]
		void GetWebView (Action<WKWebView> completion);
