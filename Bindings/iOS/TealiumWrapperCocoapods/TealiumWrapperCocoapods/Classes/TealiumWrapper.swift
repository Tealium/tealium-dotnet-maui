...
public class TealiumWrapper: NSObject {
    ...
    @objc public func gatherTrackData(_ completion: @escaping ([String: Any]) -> ()) {
        tealium.gatherTrackData(completion: completion)
    }
    
    @objc public func getWebView(_ completion: @escaping (WKWebView) -> Void) {
        tealium.getTagManagementWebView(completion)
    }
